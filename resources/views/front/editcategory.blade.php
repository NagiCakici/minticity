...
@section('content')
    <div class="conteiner">
        <div class="fs-2 text-center p-4">Kategori Düzenle</div>
        <form action="{{ route('updateCategory') }}" method="post">
            @csrf
            <input type="hidden" name="id" value="{{ $category->id }}">
            <div class="mb-3">
                    <label for="category" class="form-label">Kategori Adı:</label>
                    <input type="text" name="title" class="form-control" id="category" aria-describedby="categoryHelp" value="{{ $category->title }}">
            ...
            <div class="border-bottom row">
                ...
                <div class="col-6"><button type="submit" class="btn btn-success">Kaydet +</button></div>
            </div>
        </form>
    </div>
@endsection
